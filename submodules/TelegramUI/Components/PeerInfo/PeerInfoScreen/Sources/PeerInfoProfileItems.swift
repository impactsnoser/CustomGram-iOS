...
func infoItems(
    ...
    if case let .user(user) = data.peer {
        ...
        let ItemCustomGramSettings = 9100
        let ItemCustomGramPeerId = 9101
        let ItemCustomGramNamespace = 9102
        let ItemCustomGramDC = 9103
        ...
        if CustomGramFeatureGates.showPeerIds {
            ...
            items[currentPeerInfoSection]!.append(PeerInfoScreenLabeledValueItem(
                id: ItemCustomGramNamespace,
                label: "Namespace",
                text: "\(user.id.namespace)",
                textColor: .primary,
                action: nil,
                requestLayout: { animated in
                    interaction.requestLayout(animated)
            ...
            items[currentPeerInfoSection]!.append(PeerInfoScreenLabeledValueItem(
                id: ItemCustomGramDC,
                label: "DC",
                text: "\(context.account.network.datacenterId)",
                textColor: .primary,
                action: nil,
                requestLayout: { animated in
                    interaction.requestLayout(animated)
        ...
        if isMyProfile {
            items[.peerSettings]!.append(PeerInfoScreenActionItem(
                id: ItemCustomGramSettings,
                text: "CustomGram",
                icon: UIImage(bundleImageName: "Settings/Icons/Experimental"),
                action: {
                    if let navigationController = interaction.getController()?.navigationController as? NavigationController {
                        navigationController.pushViewController(customGramSettingsController(context: context))
                    }
                }
            ))
        }
