    ...
    for operation in optimizedOperations(finalState.state.operations) {
        switch operation {
            ...
            case let .EditMessage(id, message):
                ...
                if CustomGramFeatureGates.vaultEnabled, let previousMessage = transaction.getMessage(id), previousMessage.text != message.text {
                    MessageVaultStore.shared.recordEdit(
                        messageId: id,
                ...
                transaction.updateMessage(id, update: { previousMessage in
                    ...
                    if previousMessage.text != message.text {
                        ...
                        if CustomGramFeatureGates.vaultEnabled {
                            updatedAttributes.removeAll(where: { $0 is CustomGramEditedMessageAttribute })
                            updatedAttributes.append(CustomGramEditedMessageAttribute(originalText: previousMessage.text))
                        }
                    }
